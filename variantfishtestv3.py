...
def print_scores(scores):
    return "%d %d %d" % (scores[0], scores[1], scores[2])
...
class EngineMatch:
    ...
    def __init__(self):
        ...
        self.parser.add_argument("-d", "--depth",help="max number of plies to evaluate", type=int, default=13)
        self.parser.add_argument("-sl1", "--skill1",help="skill level for engine1", type=int, default=20)
        self.parser.add_argument("-sl2", "--skill2",help="skill level for engine2", type=int, default=20)
    ...
    def init_engines(self):
        ...
        self.info_handlers = []
        for engine, options, skill in zip(self.engines, self.engine_options, self.skills):
            engine.uci()
            if self.config:
            ...
            engine.setoption({"skill level":skill})
    ...
    def init_game(self):
        ...
        for engine in self.engines:
            engine.ucinewgame()
            engine.setoption({"clear hash": False, "UCI_Variant": self.variant})
    ...
    def play_game(self, white, black, pos="startpos"):
        ...
        while True:
            ...
            bestmove, ponder = e.go(wtime=self.wt, btime=self.bt, winc=self.inc, binc=self.inc,depth=self.depth)
